...
function GetLastBackup()
{
    ...
    then
        ...
        LastSlash=`ls -d */ | grep ".complete" | sort | tail -1`
        #Remove the last "/" to enable rsync to use it in link-dest
        Last=${LastSlash::-1}
        #Check if the directory is there
        if [ -z $Last ]
        then LastBackUp=""
        else LastBackUp="$BackupPath/$BackupFold/$HostName/$Last"
        fi
...
function GetName()
{
    ...
    then
        ...
        InComp=`ls -d */ | grep -v ".complete" | sort | tail -1`
