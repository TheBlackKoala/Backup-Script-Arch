...
function GetLastBackup()
{
    ...
    echo "$BackupPath/$BackupFold/$HostName"
...
function Backup()
{
    ...
    then
        #Nothing to link to
        rsync $opts --exclude="$excl" $BackupDirs "$BackupPath/$BackupFold/$HostName/$BackupTime"
    else
        #Link to the old backup
        rsync $opts --exclude="$excl" --link-dest=$LastBackUp $BackupDirs "$BackupPath/$BackupFold/$HostName/$BackupTime"
    fi
